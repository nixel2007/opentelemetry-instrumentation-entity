...
`opentelemetry-instrumentation-entity` подписывается на события менеджера сущностей [entity](https://github.com/nixel2007/entity) и превращает их в телеметрию [OpenTelemetry SDK](https://github.com/nixel2007/opentelemetry). Библиотека устроена как инструментирования JDBC и Hibernate в Java: ORM не зависит от OpenTelemetry и отдает события через собственный интерфейс `НаблюдательСущностей`, а этот пакет реализует наблюдателя.
...
Наблюдатель регистрируется на менеджере и видит все, что идет через его пул соединений: операции самого менеджера, его хранилищ сущностей и активной записи. Зарегистрированный до `Инициализировать` наблюдатель видит и создание таблиц.

В приложениях на Autumn регистрацию делает связка `autumn-data` и `autumn-opentelemetry`: наблюдатель создается завязью при включенном `otel.entity.enabled` и подставляется во все менеджеры сущностей.

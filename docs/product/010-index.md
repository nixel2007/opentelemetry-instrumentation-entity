...
Обертка дает только метрику. Спанов вокруг вызовов она не создает: в трейсе видны операции entity и запросы к СУБД, которые метод выполнил, а сам вызов метода виден в гистограмме `entity.repository.invocations` с атрибутами `entity.repository` и `code.function.name`.
